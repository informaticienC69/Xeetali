...
function getRegionColor(name: string, filterGroup: string|null, viewMode: ViewMode): string {
  ...
  if (viewMode==="alertes") return r.alertes===0?"#2f9e63":r.alertes<=2?"#d68a1a":"#ce3341";
  if (viewMode==="densite") { const d=(r.totalPoches/r.population)*100000; return d>15?"#2f9e63":d>8?"#d68a1a":"#ce3341"; }
  ...
  return (
    <div className="dropdown-pop backdrop-blur-xl" style={{position:"absolute",left:flip?pos.x-210:pos.x+14,top:Math.max(8,pos.y-20),width:196,background:"color-mix(in srgb, var(--surface) 80%, transparent)",border:`1px solid ${color}50`,borderRadius:12,padding:"10px 12px",boxShadow:`0 12px 40px rgba(0,0,0,0.5),inset 0 0 0 1px rgba(255,255,255,0.2)`,zIndex:100,pointerEvents:"none"}}>
      ...
      <div className="space-y-1">
        ...
        {shown.map((bg, i)=>{const p=Math.round((bg.qty/bg.max)*100);const bc=p>50?"#2f9e63":p>20?"#d68a1a":"#ce3341";return(
      ...
      <div className="flex items-center gap-3 mt-3 pt-2" style={{borderTop:"1px solid var(--line)"}}>
        ...
        {region.alertes>0&&<span className="flex items-center gap-1 mono text-[10px]" style={{color:"#ce3341"}}><AlertTriangle size={10}/> {region.alertes}</span>}
      </div>
      ...
      <div className="flex items-start justify-between px-5 py-4 shrink-0" style={{borderBottom:"1px solid var(--line)",background:`linear-gradient(135deg, ${color}10, transparent)`}}>
        <div>
          ...
          <div className="flex items-center gap-2 mt-1">
            <span className="mono text-[10px] px-2 py-0.5 rounded-md" style={{background:STATUS_CFG[region.status].bg,color,border:`1px solid ${color}40`}}>{STATUS_CFG[region.status].label}</span>
            <TI size={12} style={{color:region.trend==="up"?"#2f9e63":region.trend==="down"?"#ce3341":"var(--txt-mute)"}}/>
          </div>
        </div>
      ...
      <div className="flex-1 px-5 py-4 space-y-4 overflow-y-auto no-scrollbar">
        ...
        <div>
          ...
          <div className="space-y-2">
            {region.bloodGroups.map(bg=>{const p=Math.round((bg.qty/bg.max)*100);const bc=p>50?"#2f9e63":p>20?"#d68a1a":"#ce3341";return(
              <div key={bg.group}>
                <div className="flex items-center justify-between mb-0.5">
                  <div className="flex items-center gap-2"><span className="mono text-[11px] font-bold w-8" style={{color:"var(--txt)"}}>{bg.group}</span>{p<20&&<AlertTriangle size={10} style={{color:"#ce3341"}}/>}</div>
        ...
        <div style={{borderTop:"1px solid var(--line)",paddingTop:12}}>
          ...
          {critiques.map(r=>(
            <div key={r.name} className="flex items-center gap-2 py-1.5">
              ...
              {r.alertes>0&&<span className="mono text-[10px] flex items-center gap-0.5" style={{color:"#ce3341"}}><Zap size={10} />{r.alertes}</span>}
            </div>
          ))}
    ...
    <div className="surface overflow-hidden" style={{borderRadius:16,border:"1px solid var(--line)"}}>
        ...
        <div className="flex flex-wrap items-center gap-3">
          ...
          <div className="flex items-center gap-1">
            ...
            <div className="flex flex-wrap gap-1">
              {BLOOD_GROUPS.map(g=>(
                <button key={g} onClick={()=>{setFilterGroup(prev=>prev===g?null:g);setViewMode("stock");}} className="mono text-[10px] px-2 py-0.5 rounded-md border transition-colors" style={{borderColor:filterGroup===g?"var(--blood)":"var(--line)",background:filterGroup===g?"rgba(206,51,65,0.15)":"transparent",color:filterGroup===g?"var(--blood)":"var(--txt-mute)"}}>{g}</button>
              ))}
            </div>
          </div>
        </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-4 px-5 py-3 shrink-0" style={{borderTop:"1px solid var(--line)",background:"var(--surface-2)"}}>
        <div className="flex items-center gap-4">
          {(filterGroup?[{color:"#2f9e63",label:"> 50%"},{color:"#d68a1a",label:"20–50%"},{color:"#ce3341",label:"< 20% critique"}]:viewMode==="stock"?Object.entries(STATUS_CFG).map(([,v])=>({color:v.color,label:v.label})):viewMode==="alertes"?[{color:"#2f9e63",label:"0 alerte"},{color:"#d68a1a",label:"1–2"},{color:"#ce3341",label:"3+"}]:[{color:"#2f9e63",label:"> 15/100k"},{color:"#d68a1a",label:"8–15"},{color:"#ce3341",label:"< 8"}]).map(({color,label})=>(
